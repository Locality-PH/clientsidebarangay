...
const ItemAction = ({ id, removeId, organizationId }) => (
	<EllipsisDropdown
		menu={
			<Menu>
				<Menu.Item key="0">
...
const Item = ({ data, removeId, organizationId }) => (
	<Card>
		<Flex alignItems="center" justifyContent="between">
			<ItemHeader name={data.name} type={data.type} />
			<ItemAction id={data.id} removeId={removeId} organizationId={organizationId} />
		</Flex>
		<div className="mt-2">
			<ItemInfo
			...
			<Row gutter={16}>
				{list.map(result => (
					<Col xs={24} sm={24} md={24} lg={12} xl={12} xxl={12} key={result.id}>
						<Item data={result} removeId={id => deleteItem(id)} organizationId={organizationId} />
					</Col>
				))}
			</Row>
		</>
